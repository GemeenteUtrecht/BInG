...
1. ``bing-django``: backend tests, runs ``./bin/jenkins_django.sh``.
2. ``bing-js``: frontend tests, runs ``./bin/jenkins_js.sh``.

**develop** branch
------------------

1. ``bing-django-develop``: backend tests, runs ``./bin/jenkins_django.sh``.
2. ``bing-django-develop-js``: frontend tests, runs ``./bin/jenkins_js.sh``.

pull requests
-------------
1. ``bing-pr-django``: backend tests, runs ``./bin/jenkins_django_pr.sh``.
2. ``bing-pr-js``: frontend tests, runs ``./bin/jenkins_js.sh``.
3. ``bing-pr-sass``: checks that sass compiles, runs ``./bin/jenkins_sass.sh``.
4. ``bing-pr-isort``: checks that imports are correctly
