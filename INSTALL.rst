...
2. Get the code:
   ...
   .. code-block:: bash
       ...
       $ git clone git@bitbucket.org:maykinmedia/bing.git
       $ cd bing
...
``src/bing/conf/local.py``. You can base this file on the
...
.. code-block:: bash
    ...
    $ python src/manage.py test bing
...
* ``DB_NAME``: name of the database for the project. Defaults to ``bing``.
* ``DB_USER``: username to connect to the database with. Defaults to ``bing``.
* ``DB_PASSWORD``: password to use to connect to the database. Defaults to ``bing``.
...
2. Start the database and web services:
   ...
   .. code-block:: bash
       ...
       $ docker-compose up -d
       Starting bing_db_1 ... done
       Starting bing_web_1 ... done
...
3. Create an admin user and load initial data. If different container names
   ...
   .. code-block:: bash
       ...
       $ docker exec -it bing_web_1 /app/src/manage.py createsuperuser
       Username: admin
       ...
       Superuser created successfully.

       $ docker exec -it bing_web_1 /app/src/manage.py loaddata admin_index groups
...
.. _Github: https://github.com/maykinmedia/bing/
...
environment variables. See ``src/bing/conf/docker.py`` for
all settings.
...
.. code-block:: bash
    ...
    $ docker exec -it bing /app/src/manage.py createsuperuser
